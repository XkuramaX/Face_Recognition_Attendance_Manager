...
def give_attendance(roll):
    print("Here",roll,type(roll))
    roll = str(roll)
    try:
        resp = db.users.find_one({"roll_number":roll})
        print(resp)
        resp = dict(resp)
        if(resp):
            attendance = {
            ...
            dbResponse = db.attendance.insert_one(attendance)
